...
int main(int argc,char *argv[]) {
    ...
    auto computeF0 = [&](const double x0, const double *x, const double sum_alpha, const double sum_beta)
    {
        return vec_Alpha[1] * x[2] - x0 * (2*vec_Beta[1]*x0 - 1 - k) + sum_alpha - sum_beta;
    };
    ...
    while (t < TIME) {
        ...
        if (myid == 0) {
            vec_Fc[0] = computeF0(C0, vec_C, vec_tot_AB[0], vec_tot_AB[4]);
        }
        ...
        if (myid == 0) {
            vec_Fa[0] = computeF0(A0, vec_A, vec_tot_AB[1], vec_tot_AB[5]);
        }
        ...
        if (myid == 0) {
            vec_Fb[0] = computeF0(B0, vec_B, vec_tot_AB[2], vec_tot_AB[6]);
        }
        ...
        if (myid == 0) {
            vec_Fd[0] = computeF0(D0, vec_D, vec_tot_AB[3], vec_tot_AB[7]);
        }
