...
// Vector with a leading space
template <typename FLOAT>
struct ShiftVec {
    const size_t shift = 0;
    FLOAT *data = nullptr;

    ShiftVec(size_t _count, size_t _shift = 0)
        : shift(_shift)
    {
        data = (FLOAT *)calloc(_count + _shift, sizeof(FLOAT));
    }
...
void computeSumAlphaBeta(double &sum_alpha, double &sum_beta,
                         ...
                         const int n,
                         const int rank = 0) {
    #pragma omp simd reduction(+:sum_alpha,sum_beta)
    for (int i = 0; i < n; i++) {
...
int getBlockSize(int N, int comm_rank, int comm_size) {
    int first = comm_rank * N / comm_size;
    int last = (comm_rank + 1) * N / comm_size - 1;
    return last - first + 1;
}
...
int main(int argc,char *argv[]) {
    ...
    // MPI rank neighbors and communication tags
    int left = myid - 1;
    int right = myid + 1;
    int tag1=1,tag2=2,tag3=3,tag4=4,tag5=5,tag6=6,tag7=7,tag8=8,tag_a=9,tag_b=10;

    // Decomposition
    int n = getBlockSize(N, myid, numprocs);
    printf("[Rank %d] block size n = %d\n", myid, n);
    ...
    ShiftVec<double> vec_Fa(n, 1);
    ShiftVec<double> vec_Fb(n, 1);
    ShiftVec<double> vec_Fc(n, 1);
    ShiftVec<double> vec_Fd(n, 1);
    ...
    MPI_Sendrecv(&vec_Beta[n], 1, MPI_DOUBLE, right, tag_b,
                 &vec_Beta[0], 1, MPI_DOUBLE, left, tag_b, comm, &status);
    MPI_Sendrecv(&vec_Alpha[0], 1, MPI_DOUBLE, left, tag_a,
                 &vec_Alpha[n], 1, MPI_DOUBLE, right, tag_a, comm, &status);
    ...
    while (t < TIME) {
        ...
        computeSumAlphaBeta(vec_AB[0], vec_AB[4],
            vec_Alpha, vec_Beta, vec_C, C0, n, myid);

        MPI_Reduce(vec_AB, vec_tot_AB, 8, MPI_DOUBLE, MPI_SUM, 0, comm);
        MPI_Sendrecv(&vec_C[n],1,MPI_DOUBLE,right,tag1,&vec_C[0],1,MPI_DOUBLE,left,tag1,comm,&status);
        MPI_Sendrecv(&vec_C[1],1,MPI_DOUBLE,left,tag2,&vec_C[n+1],1,MPI_DOUBLE,right,tag2,comm,&status);

        // Compute F_C(t)
        if (myid == 0) {
            vec_Fc[0] = C0 * (-2*vec_Beta[1]*C0 + 1 + k) -vec_tot_AB[4]+vec_tot_AB[0]+vec_Alpha[1]*vec_C[2];
        }
        ...
        // Compute the sum of alpha and beta
        computeSumAlphaBeta(vec_AB[1], vec_AB[5],
            vec_Alpha, vec_Beta, vec_A, A0, n, myid);

        MPI_Reduce(vec_AB, vec_tot_AB, 8, MPI_DOUBLE, MPI_SUM, 0, comm);
        MPI_Sendrecv(&vec_A[n],1,MPI_DOUBLE,right,tag3,&vec_A[0],1,MPI_DOUBLE,left,tag3,comm,&status);
        MPI_Sendrecv(&vec_A[1],1,MPI_DOUBLE,left,tag4,&vec_A[n+1],1,MPI_DOUBLE,right,tag4,comm,&status);

        // Compute F_A(t)
        if (myid == 0) {
            vec_Fa[0] = -2*vec_Beta[1]*A0*A0-vec_tot_AB[5]+vec_tot_AB[1]+vec_Alpha[1]*vec_A[2]+(1+k)*A0;
        }
        ...
        // Compute the sum of alpha and beta
        computeSumAlphaBeta(vec_AB[2], vec_AB[6],
            vec_Alpha, vec_Beta, vec_B, B0, n, myid);

        MPI_Reduce(vec_AB, vec_tot_AB, 8, MPI_DOUBLE, MPI_SUM, 0, comm);
        MPI_Sendrecv(&vec_B[n],1,MPI_DOUBLE,right,tag5,&vec_B[0],1,MPI_DOUBLE,left,tag5,comm,&status);
        MPI_Sendrecv(&vec_B[1],1,MPI_DOUBLE,left,tag6,&vec_B[n+1],1,MPI_DOUBLE,right,tag6,comm,&status);

        // Compute F_B(t)
        if (myid == 0) {
            vec_Fb[0] = -2*vec_Beta[1]*B0*B0-vec_tot_AB[6]+vec_tot_AB[2]+vec_Alpha[1]*vec_B[2]+(1+k)*B0;
        }
        ...
        // Compute the sum of alpha and beta
        computeSumAlphaBeta(vec_AB[3], vec_AB[7],
            vec_Alpha, vec_Beta, vec_D, D0, n, myid);

        MPI_Reduce(vec_AB, vec_tot_AB, 8, MPI_DOUBLE, MPI_SUM, 0, comm);
        MPI_Sendrecv(&vec_D[n],1,MPI_DOUBLE,right,tag7,&vec_D[0],1,MPI_DOUBLE,left,tag7,comm,&status);
        MPI_Sendrecv(&vec_D[1],1,MPI_DOUBLE,left,tag8,&vec_D[n+1],1,MPI_DOUBLE,right,tag8,comm,&status);

        // Compute F_D(t)
        if (myid == 0) {
            vec_Fd[0] = -2*vec_Beta[1]*D0*D0-vec_tot_AB[7]+vec_tot_AB[3]+vec_Alpha[1]*vec_D[2]+(1+k)*D0;
        }
